...
async function GetNewLeads() {
  ...
  // if (!session) {
  //   redirect("/api/auth/signin?callbackUrl=/see-leads");
  // }
    ...
    <tr key={lead.id}>
      {Object.entries(lead)
        ...
        .map(([key, value]) => (
          <td
            key={key}
            className="text-gray-200 px-4 py-2 border border-gray-300 text-left hover:transform hover:scale-105 transition-transform"
          >
            {key === "contacted" ? (value ? "Yes" : "No") : value}{" "}
      ...
      <td
        key="edit"
        className="px-4 py-2 border text-gray-200 underline border-gray-300 text-left"
      >
        <Link href={`/edit/${lead.id}`}>Edit</Link>
      </td>
    </tr>
  ));
